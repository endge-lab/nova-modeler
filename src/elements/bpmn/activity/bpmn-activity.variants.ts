...
export function resolveBpmnActivityKindIcon(kind: BpmnActivityKind) {
  if (kind === 'subProcess') {
    return MODELER_ASSETS.icons.activitySubProcess
  }
  if (kind === 'eventSubProcess') {
    return MODELER_ASSETS.icons.activityEventSubProcess
  }
  if (kind === 'transaction') {
    return MODELER_ASSETS.icons.activityTransaction
  }
  if (kind === 'adHocSubProcess') {
    return MODELER_ASSETS.icons.activityAdHoc
  }
  if (kind === 'callActivity') {
    return MODELER_ASSETS.icons.activityCall
  }
...
export function resolveBpmnTaskTypeIcon(taskType: BpmnTaskType) {
  if (taskType === 'user') {
    return MODELER_ASSETS.icons.taskUser
  }
  if (taskType === 'manual') {
    return MODELER_ASSETS.icons.taskManual
  }
  if (taskType === 'service') {
    return MODELER_ASSETS.icons.taskService
  }
  if (taskType === 'script') {
    return MODELER_ASSETS.icons.taskScript
  }
  if (taskType === 'businessRule') {
    return MODELER_ASSETS.icons.taskBusinessRule
  }
  if (taskType === 'send') {
    return MODELER_ASSETS.icons.taskSend
  }
  if (taskType === 'receive') {
    return MODELER_ASSETS.icons.taskReceive
  }
  return undefined
}
